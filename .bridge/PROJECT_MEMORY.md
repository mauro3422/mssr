...
The 0.2.10 portable core (strict producers, repository collector, freshness revalidation, and a durable explicit-ack advisory-only JSON inbox) proved the portable message plane and repository scan. 0.2.11 adds keyed repository facts (selectors, source-kind defaults, optional `.bridge/context-messages.json` manifest), the modular `.bridge/project-context-modules.json` loader, and wires the durable plane into native, Codex, and OpenCode route/bootstrap through `loadProjectContextHost` plus explicit `mssr_context_ack`. 0.2.12 makes an acknowledged receipt a temporary tombstone for identical evidence: enqueue suppresses only acknowledged same-`messageId` plus same-content-`fingerprint` messages, content/revision changes or a new id reappear, and `receiptRetentionMs` pruning re-enables identical delivery; the inbox schema is v2 with transparent v1 migration. 0.2.13 tightens `godot-graph-ux-audit` to its `godot` identity domain and adds explicit `complex-system-design` ownership. 0.2.14 records a separate durable boundary for user-owned `godot-scene-authoring`: scene open/create/save/editor-state changes are authoring work and depend on `godot-project-inspection`, while read-only scene review does not activate authoring. Portable MSSR owns only the selection metadata/fixtures; native scene tool contracts remain host/provider responsibilities.

## Bridge delivery boundary

Bridge must consume MSSR through a packaged artifact rather than an in-place junction across workspace authority boundaries. Bridge `0.6.91` now vendors MSSR `0.2.14`, so the adapter receives the current context-plane behavior plus explicit `godot-scene-authoring` routing metadata/fixtures from a versioned package. Future MSSR source changes are not live in Bridge until a new packaged artifact is deliberately installed and the Bridge runtime is restarted/read back; never infer live adoption from the sibling source tree alone.
