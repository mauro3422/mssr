...
- Added strict durable `learning-digest-v1` telemetry for outcome-time distillation. It persists only semantic routing/context selections, stage-to-skill transitions, final outcome metadata, and evidence-backed supported/rejected findings; ephemeral `workingSummary`, active hypotheses, raw prompts/transcripts, secrets, and private reasoning remain excluded.
- Added minimum-evidence historical priors by exact semantic signature for skill activation/acceptance, stage transitions, and skill/project context modules. The learning surface is explicitly `observe-only` with `routingInfluence:false`: priors expose empirical rates plus `prefer`, `neutral`, `deprioritize`, or `insufficient-evidence` labels but do not affect routing, scores, context loading, or permissions.
