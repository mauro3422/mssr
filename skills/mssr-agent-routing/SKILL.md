...
- When a host exposes bounded change metadata, let portable MSSR classify project-knowledge maintenance separately from routing: `WATCH` stays low-noise evidence; `REVIEW`/`REQUIRED` is a close/replan signal to load `skill-maintenance-loop` plus only the named `.mssr` authority/module. Before writing a Markdown source referenced by `.mssr/project-context.json`, use Project Context budget preflight when the host can materialize the proposed next text: projected `REVIEW` replans into `skill-maintenance-loop` before the write, and a proposed payload above the declared `maxChars` must be narrowed/split rather than hidden by raising the budget. Critical cross-cutting project rules must not be made semantically broad merely to force retrieval: when repository review proves a rule must apply to a class of mutations, encode that bounded applicability as `requiredWhen.mutation=true` with an optional artifact scope so it is reserved before semantic ranking while read-only work stays selective. Trace data, diffs, runtime/package state and freshness may trigger review but never become the durable fact themselves.
