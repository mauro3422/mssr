...
- **Read when:** Project Context Health reports growing/oversized PROJECT_* authorities or modules, unindexed `.mssr/knowledge/`, or the project-context core is carrying situational detail.
- **Status:** active
...
- `PROJECT_MEMORY.md` = compact durable decisions/lessons;
- `PROJECT_STATE.md` = compact current state/handoff;
- `.mssr/knowledge/<topic>/` = situational project detail selected by `.mssr/project-context.json`;
- `.mssr/runtime/` = reconstructable state, never project truth.

Semantic `topic` (`architecture`, `design`, `law`, `pattern`, `vocabulary`, `decision`, `state`, `phase`, `reference`, `operations`, `other`) is independent from physical `kind` (`context`, `memory`, `state`, `directive`). `area` optionally narrows the subsystem.

## Decision ladder

1. Run Project Context Health and freeze the relevant manifest/source hashes.
2. If the finding is only `WATCH`, do not interrupt unrelated work; schedule/review during maintenance.
3. Run the portable modularization planner.
...
5. Verify selected-context behavior before/after. Lower byte count is not success if the relevant task stops receiving required knowledge.
